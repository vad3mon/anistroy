...
class AuthorsRequestsController extends Controller
{
    ...
    public function filesList(Request $request)
    {
        ...
        if($request->input('big')) {
            switch($request->input('big')) {
                case 'no_big':
                    $authorRequestFiles = $authorRequestFiles->where('author_request_files.is_big', '0');
                break;
                case 'only_big':
                    $authorRequestFiles = $authorRequestFiles->where('author_request_files.is_big', '1');
                break;
            }
        }
        ...
        if($request->input('chief')) {
            switch($request->input('chief')) {
                case 'yes':
                    $authorRequestFiles = $authorRequestFiles->where('author_request_files.chief_editor_mark', 'yes');
                break;
                case 'no':
                    $authorRequestFiles = $authorRequestFiles->where('author_request_files.chief_editor_mark', 'no');
                break;
                case 'think':
                    $authorRequestFiles = $authorRequestFiles->where('author_request_files.chief_editor_mark', 'think');
                break;
            }
        }
        ...
        if($request->input('voted')) {
            $allVotes = DB::table('editor_read_results')->select('author_file_id')->where('user_id', $user->id)->get()->pluck('author_file_id');

            switch($request->input('voted')) {
                case 'yes':
                    $authorRequestFiles = $authorRequestFiles->whereIn('author_request_files.id', $allVotes);
                break;
                case 'no':
                    $authorRequestFiles = $authorRequestFiles->whereNotIn('author_request_files.id', $allVotes);
                break;
            }
        }
    ...
    public function filesItem($id)
    {
        ...
        $authorRequestFile = AuthorRequestFile::where('author_request_files.id', $id)
            ...
            ->select('author_request_files.*',
                ...
                'author_requests.contact as author_contacts',
        ...
        $editorComments = EditorReadComment::where('author_file_id', '=', $id)
            ->join('users', 'users.id', '=', 'editor_read_comments.user_id')
            ->select('users.name as username', 'editor_read_comments.*')
            ->get();
        ...
        $editorDecisions = EditorReadDecision::where('author_file_id', '=', $id)
            ->join('users', 'users.id', '=', 'editor_read_decisions.user_id')
            ->select('users.name as username', 'editor_read_decisions.*')
            ->get();
